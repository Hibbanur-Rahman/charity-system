...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST["signup"])) {
        ...
        $sql = "INSERT INTO `volunteer_info_login` (`username`,`password`,`mobile`,`email`,`address`) VALUES ('$username','$password','$mobile','$email','$address')";
    ...
    if (isset($_POST["login"])) {
        ...
        if (empty($email) || empty($password)) {
            ...
            //query template
            $sql = "SELECT `email`,`password`,`username` from `volunteer_info_login` where `email`=? and `password`=?";

            $sql1 = "SELECT `email`,`password`,`username` from `volunteer_info_login` where `email`='$email' and `password`='$password'";
