...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST["signup"])) {
        ...
        $min=10000;
        $max=99999;
        $donorid = mt_rand($min, $max);

        $sql = "INSERT INTO `donor_info_login` (`username`,`password`,`mobile`,`email`,`address`, `registered date and time`,`donor id`) VALUES ('$username','$password','$mobile','$email','$address',NOW(),'$donorid')";
    ...
    if (isset($_POST["login"])) {
        ...
        if (empty($email) || empty($password)) {
            ...
            //query template
            $sql = "SELECT `email`,`password`,`username`,`donor id` from `donor_info_login` where `email`=? and `password`=?";

            $sql1 = "SELECT `email`,`password`,`username`,`donor id` from `donor_info_login` where `email`='$email' and `password`='$password'";
            ...
            if($stmt=mysqli_prepare($conn,$sql)){
                ...
                if(mysqli_stmt_num_rows($stmt)==1){
                    session_start();
                    $row = $qry->fetch_assoc();
                    $_SESSION['DonarId'] = $row['username'];
                    $_SESSION['email']=$row['email'];
                    $_SESSION['ActualdonorId']=$row['donor id'];
                    echo $_SESSION['DonarId'];
                    header("location: ../../donar_panel/index.php");
                    exit;
                }
                else{
